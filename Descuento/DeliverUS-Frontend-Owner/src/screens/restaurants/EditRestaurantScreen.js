...
export default function EditRestaurantScreen ({ navigation, route }) {
  ...
  const [restaurantToBeUpdate, setRestaurantToBeUpdate] = useState(false)
  ...
  const updateRestaurant = async (values) => {
    setBackendErrors([])
    if (values.percentage != 0 && !restaurantToBeUpdate) {
      setRestaurantToBeUpdate(true)
    } else {
      setRestaurantToBeUpdate(false)
      try {
    ...
    <Formik
      ...
      {({ handleSubmit, setFieldValue, values }) => (
        <ScrollView>
          <View style={{ alignItems: 'center' }}>
            <View style={{ width: '60%' }}>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginTop: 20, marginBottom: 10 }}>
                ...
                <TextSemiBold>Porcentage actual: <TextSemiBold textStyle={{ color: GlobalStyles.brandPrimary }}>{values.percentage.toFixed(1)}%</TextSemiBold></TextSemiBold>
          ...
          <ConfirmationModal
            isVisible={restaurantToBeUpdate}
            onCancel={() => setRestaurantToBeUpdate(false)}
