...
const checkDate = async (value, { req }) => {
  try {
    if (value && value >= new Date()) {
      ...
      return Promise.reject(new Error('Solo puede ser la fecha posterior al dia de hoy'))
    }
  } catch (err) {
    return Promise.reject(new Error(err))
  }
}
const checkAva = async (value, { req }) => {
  try {
    if (value === false && req.body.visibleUntil != null) {
      return Promise.reject(new Error('Solo pueden ponerse visibles aquellos productos que tengan disponibilidad.'))
    } else {
      return Promise.resolve()
    }
  } catch (err) {
...
const create = [
  ...
  check('visibleUntil').custom(checkDate),
  check('availability').custom(checkAva)
]
