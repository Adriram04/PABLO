...
export default function RestaurantsScreen ({ navigation, route }) {
  ...
  const [restaurantToPin, setRestaurantToPin] = useState(null)
  ...
  const renderRestaurant = ({ item }) => {
    return (
      ...
      >
        ...
        <View sytle={{ flexDirection: 'row', justifyContent: 'space_between', alignItems: 'flex-end' }}>
          <TextSemiBold>Shipping: <TextSemiBold textStyle={{ color: GlobalStyles.brandPrimary }}>{item.shippingCosts.toFixed(2)}€</TextSemiBold></TextSemiBold>
          <Pressable onPress={() => setRestaurantToPin(item)}>
  ...
  const pinear = async (restaurant) => {
    try {
      await pin(restaurant.id)
      await fetchRestaurants()
      setRestaurantToPin(null)
      showMessage({
  ...
  return (
    ...
    <ConfirmationModal
      isVisible={restaurantToPin !== null}
      onCancel={() => setRestaurantToPin(null)}
      onConfirm={() => pinear(restaurantToPin)}>
    </ConfirmationModal>
    </>
  )
}
